...
FMOD_BOOL CSoundMgr::IsChannelPlaying(CHANNELID eID)
{
	FMOD_BOOL bPlay;
...
void CSoundMgr::SetChannelFrequency(CHANNELID eID, float _num)
{
	float originalFrequency;
	FMOD_Channel_GetFrequency(m_pChannelArr[eID], &originalFrequency);
	FMOD_Channel_SetFrequency(m_pChannelArr[eID], originalFrequency * _num);
}

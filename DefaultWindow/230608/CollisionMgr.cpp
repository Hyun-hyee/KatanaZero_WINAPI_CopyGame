...
DIR CCollisionMgr::Collision_Enter_EE(INFO* _first, INFO* _second)
{
...
DIR CCollisionMgr::Collision_Enter_ES(INFO* _first, RECT* _second)
{
...
DIR CCollisionMgr::Collision_Enter_SE(RECT* _second, INFO* _first)
{
	float DistanceList[4] = {};
	bool Check = false;
	DistanceList[0] = sqrt(pow((_first->fX - _second->left), 2.f) + pow((_first->fY - _second->top), 2.f));
